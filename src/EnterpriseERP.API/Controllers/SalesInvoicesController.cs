...
public class SalesInvoicesController : ControllerBase
{
    ...
    {
        var success = await _mediator.Send(new EnterpriseERP.Application.Features.SalesInvoices.Commands.ApproveSalesInvoice.ApproveSalesInvoiceCommand(id));

        return Ok(new
        {
            Success = success,
            Message = success ? "Sales invoice approved and posted successfully" : "Failed to approve invoice",
            Data = success,
            Errors = Array.Empty<string>()
        });
    }
