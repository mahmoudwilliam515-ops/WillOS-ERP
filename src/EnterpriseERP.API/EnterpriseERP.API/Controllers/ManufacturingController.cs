...
public class ManufacturingController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateOrder([FromBody] CreateProductionOrderCommand command)
    {
        ...
        if (!result.IsSuccess) return BadRequest(new { success = false, message = result.Error.Message });
        return Ok(new { success = true, data = result.Value, message = "Order created" });
    }
